...
bbox = {'top': 50, 'left': 10, 'width': 600, 'height': 400}
...
def draw_lanes(img, lines, color=[255, 255, 255], thickness=1):
...
while 1:
    #capture real time data from screen
    ...
    GRAYimg= cv2.cvtColor(HSLimg, cv2.COLOR_BGR2GRAY)
    #find the Lchannel
    Lchannel = HSLimg[:,:,1]
    ...
    region= np.array([[10, 440], [10, 460], [260, 220], [360, 220], [670, 460], [670, 440],], np.int32)
    ...
    lines = cv2.HoughLinesP(processed_img_region, 2, np.pi/180, 40, 30, 20)

    try:
        l1, l2 = draw_lanes(sct_img,lines)
        cv2.line(sct_img, (l1[0], l1[1]), (l1[2], l1[3]), [255,255,255], 10)
        cv2.line(sct_img, (l2[0], l2[1]), (l2[2], l2[3]), [255, 255, 255], 10)
    except Exception as e:
        print(str(e))
        pass
    try:
        for coords in lines:
            coords = coords[0]
            try:
                cv2.line(sct_img, (coords[0], coords[1]), (coords[2], coords[3]), [0,0, 255], 1)
